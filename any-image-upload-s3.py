...
def process_images(folder_path):
    """Loops through all supported images in a folder and uploads them to S3."""
    s3_urls = []
    for root, _, files in os.walk(folder_path):
        for file_name in files:
            if file_name.lower().endswith(('.jpeg', '.jpg', '.png', '.webp')):
...
if __name__ == "__main__":
    folder_path = "/Users/amir/Desktop/Idea-homepage"  # Path to the folder containing images
    output_file = "homepage_images.xlsx"  # Output Excel file name
    if os.path.exists(folder_path):
        print("Processing images...")
        s3_urls = process_images(folder_path)
        print("\nAll uploaded images:")
